...
PACKED_OFFSET_BEGIN = 0x400   # where packed file begins in the packer
PACKED_OFFSET_END   = 0x3000  # 0x3000 is enough to get the decrypted secret; if we wanna unpack the WHOLE program we can use offset --> 0x48e70
IV_OFFSET           = 0x48E80 # hardcoded IV is located in the packer, just after the end of the packed data
...
def parseSecret(data):
    ...
    secret = data[index+12:index+60] # read large amount of data just in the case where length of secret would change
...
def decryptPackedFile(filename):
    ...
    for key in keys:
        decrypted = decryptCbc(key, iv, packed)
        
        # check if it's indeed the unpacked data we're looking for!
        if decrypted[:3] == b"MZ\x90":
