...
def main():
    # Template given by challmaker (to perform Proof-of-work)
    r = remote(HOST, PORT)
    _ = r.recvlines(5)
    ...
    while True:
        ...
        filenamePacked = "/tmp/hyperpacker%d.exe" % (n)
        with open(filenamePacked, "wb") as fp:
            fp.write(binary)

        log.success("Binary written at " + filenamePacked)
        print(r.recv())
        
        # Decrypt the file
        decryptedMzPe = decryptPackedFile(filenamePacked)
        filenameUnpacked = "/tmp/unpacked%d.exe" % (n)
        
        with open(filenameUnpacked, "wb") as f:
            f.write(decryptedMzPe)
        log.success("Unpacked binary extracted to " + filenameUnpacked)
        ...
        # Check if we receive "well done" or an error (because too long or bad secret)
        ret = r.recvline()
        print(ret)
        if b"Well done" in ret:
            pass # good
        elif b"Error" in ret:
            break # shit, we exit
        n+=1
        
        # Print the flag and if found, exit
        ret = r.recvline()
        if b"Here is your binary:" in ret:
                print(ret)
        else:
                print("Flag is:", ret) # if not displays "here is your binary", maybe it's printing us the flag?! :)
                break
